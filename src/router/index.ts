...
/**
 * 数组处理成多级嵌套数组（只保留二级：也就是二级以上全部处理成只有二级，keep-alive 支持二级缓存）
 * @description isKeepAlive 处理 `name` 值，进行缓存。顶级关闭，全部不缓存
 * @param arr 处理后的路由菜单数组
 * @returns 返回将数组重新处理成 `定义动态路由（dynamicRoutes）` 的格式
 */
  ...
  const newArr: any = []
	...
	arr.forEach((v: any) => {
        ...
        children: []
			...
			// 存 name 值，keep-alive 中 include 使用，实现路由的缓存; 路径：/@/layout/routerView/parent.vue
